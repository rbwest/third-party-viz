...
export function validateRoles(resultData, expectedTypes, optionalTypes) {
  ...
  if (numCols > expectedTypes.length) {
    ...
    if (typeof optionalTypes == "object") {
      ...
      for (
        var c = expectedTypes.length, i = 0;
        c < numCols && i < optionalTypes.length;
        c++, i++
      ) {
        ...
        for (var r = 0; r < arrayData.length; r++) {
          ...
          if (colInfo.format && colInfo.format.formatString == "DDMMYY8") {
            dateStr =
              ...
              dateStr.substr(3, 2) +
              "-" +
              dateStr.substr(0, 2); // = YYYY-MM-DD (international standard)
          } else if (
            colInfo.format &&
            colInfo.format.formatString == "JULIAN7"
          ) {
...
export function translateFormat(formatString) {
...
export function getId(str) {
  return "id-" + str.replace(/[\W]/g, "_");
}

export function modifyColor(color, percent) {
  const hexString = d3.color(color).hex();
  const clamp = d3
    .scaleLinear()
    .domain([0, 255])
    .range([0, 255])
    .clamp(true); // Clamping function
  const colorNum = parseInt(hexString.slice(1), 16); // Base 10 color number
  const delta = Math.round(2.55 * percent); // Amount to change each RGB compontent by

  // Calculate new RGB compontents
  const R = clamp((colorNum >> 16) + delta);
  const G = clamp(((colorNum >> 8) & 0x00ff) + delta);
  const B = clamp((colorNum & 0x0000ff) + delta);

  // Return color parsed back into hex
